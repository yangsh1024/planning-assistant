...
public class AgentToolExecutor {
    private final PlanService planService; private final ExpenseService expenseService;
    private final AgentActionService actionService; private final CategoryService categoryService; private final ObjectMapper objectMapper;
    ...
    public ToolResult execute(String sessionId, String name, String arguments) {
        try {
            ...
            Object result = switch (name) {
                ...
                case "read_expenses" -> expenseService.listByMonth(text(args, "yearMonth"), nullableLong(args, "categoryId"), boundedInteger(args, "page", 1, 1, 10_000), boundedInteger(args, "pageSize", 20, 1, 100));
        ...
        } catch (BizException e) { throw e; } catch (Exception e) { throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "工具参数不合法"); }
    }
    private void validateShape(String name, JsonNode args) {
        if (args == null || !args.isObject()) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "工具参数必须是对象");
        java.util.Set<String> allowed = switch (name) {
            ...
            case "prepare_update_expense" -> java.util.Set.of("expenseId", "categoryId", "amount", "expenseDate", "note");
        ...
        args.fieldNames().forEachRemaining(field -> { if (!allowed.contains(field)) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "工具参数包含未知字段：" + field); });
        if ("prepare_save_plan".equals(name) && args.path("items").isArray()) {
            args.path("items").forEach(item -> {
                ...
                item.fieldNames().forEachRemaining(field -> { if (!java.util.Set.of("categoryId", "amount", "sortOrder").contains(field)) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "预算明细包含未知字段：" + field); });
            });
        }
    }
    private AgentActionDto pending(String sessionId, String type, String summary, JsonNode args) { validatePending(type, args); return actionService.createPending(sessionId, type, readableSummary(type, summary, args), args); }
    private void validatePending(String type, JsonNode args) {
        if ("CREATE_EXPENSE".equals(type) || "UPDATE_EXPENSE".equals(type)) {
            if ("UPDATE_EXPENSE".equals(type)) {
                ...
                if (!args.hasNonNull("categoryId") && !args.hasNonNull("amount") && !args.hasNonNull("expenseDate") && !args.hasNonNull("note")) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "编辑内容不能为空");
            }
            if ("CREATE_EXPENSE".equals(type) || args.hasNonNull("amount")) MoneyValidator.parsePositiveAmount(text(args, "amount"));
            if ("CREATE_EXPENSE".equals(type) || args.hasNonNull("expenseDate")) {
                java.time.LocalDate date; try { date = java.time.LocalDate.parse(text(args, "expenseDate")); } catch (Exception e) { throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "日期格式不正确"); }
                if (date.isAfter(java.time.LocalDate.now())) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "不允许记录未来开支");
            }
            if ("CREATE_EXPENSE".equals(type) || args.hasNonNull("categoryId")) {
                long categoryId = args.path("categoryId").asLong(-1); boolean available = categoryService.listAvailable().stream().anyMatch(c -> c.getCategoryId() == categoryId);
                if (!available) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "科目不可用");
            }
            if (args.hasNonNull("note") && args.get("note").asText().length() > 100) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "备注过长");
        } else if ("SAVE_PLAN".equals(type)) {
            text(args, "yearMonth"); if (!args.has("items") || !args.get("items").isArray() || args.get("items").isEmpty()) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "预算明细不能为空");
            ...
            args.get("items").forEach(item -> {
                ...
                if (!categoryIds.add(categoryId)) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "预算科目不能重复");
                if (!availableIds.contains(categoryId)) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "科目不可用");
                MoneyValidator.parsePositiveAmount(text(item, "amount"));
                int sortOrder = boundedInteger(item, "sortOrder", -1, 0, 10_000);
                if (!sortOrders.add(sortOrder)) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "预算排序不能重复");
            });
        } else if ("CREATE_CATEGORY".equals(type)) {
            String name = text(args, "name"); if (name.length() > 20) throw new BizException(ErrorCode.PARAM_ERROR.getCode(), "科目名称过长");
    ...
    private String readableSummary(String type, String fallback, JsonNode args) {
        return switch (type) {
            case "CREATE_EXPENSE" -> "新增开支：" + categoryName(args.get("categoryId").asLong()) + " " + text(args, "amount") + " 元，" + text(args, "expenseDate") + note(args);
            case "UPDATE_EXPENSE" -> updateSummary(args);
            case "DELETE_EXPENSE" -> "删除开支：" + expenseSummary(expenseService.getById(args.get("expenseId").asLong()));
    ...
    private String expenseSummary(ExpenseDto expense) { return expense.getCategoryName() + " " + expense.getAmount() + " 元，" + expense.getExpenseDate() + optionalNote(expense.getNote()); }
    private String categoryName(long categoryId) { return categoryService.listAvailable().stream().filter(item -> item.getCategoryId() == categoryId).map(CategoryDto::getName).findFirst().orElse("科目#" + categoryId); }
    private String note(JsonNode args) { return args.hasNonNull("note") ? optionalNote(args.get("note").asText()) : ""; }
    private String optionalNote(String note) { return note == null || note.isBlank() ? "" : "，备注：" + note; }
    private String text(JsonNode args, String field) { if (!args.hasNonNull(field) || args.get(field).asText().isBlank()) throw new BizException(ErrorCode.PARAM_ERROR); return args.get(field).asText(); }
    private Long nullableLong(JsonNode args, String field) { return args.hasNonNull(field) ? positiveLong(args, field) : null; }
    private long positiveLong(JsonNode args, String field) { if (!args.hasNonNull(field) || !args.get(field).canConvertToLong() || args.get(field).asLong() <= 0) throw new BizException(ErrorCode.PARAM_ERROR); return args.get(field).asLong(); }
    private int boundedInteger(JsonNode args, String field, int fallback, int min, int max) { int value = args.hasNonNull(field) && args.get(field).canConvertToInt() ? args.get(field).asInt() : fallback; if (value < min || value > max) throw new BizException(ErrorCode.PARAM_ERROR); return value; }
